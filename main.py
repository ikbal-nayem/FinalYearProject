...
from RPi_Action import RPiAction
...
class Main:
    ...
    def checkFaces(self, faces, frame):
        ...
        for face in faces['faces']:
            ...
            if float(confidence) > MIN_CONF_LEVEL:
                ...
                print('[\033[1;32mSUCCESS\033[0;0m]\033[32m({} {}%) unlocking the system for {} sec\033[0m'.format(
                    face['top_prediction']['label'], confidence, UNLOCK_TIME))
                timer.start()
                break
        if not AUTHORIZED:
            self.attempt += 1
            print(
                '[\033[1;33mFAILED\033[0;0m] {} Unknown face/s detected!'.format(len(faces['faces'])))
    ...
    def __call__(self):
        while True:
            # if not AUTHORIZED and self.rpi.isInDoorStep():
